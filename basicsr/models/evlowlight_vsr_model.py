...
class EvLowLightVSR_Model(VideoBaseModel):
    ...
    def feed_data(self, data):
        ...
        self.voxels = data['voxels'].to(self.device)
        if 'key' in data:
            self.key = data['key']
        if 'gt' in data:
    ...
    def optimize_parameters(self, current_iter):
        if self.fix_flow_iter:
            logger = get_root_logger()
            if current_iter == 1:
                logger.info(f'Fix flow network, retinex model, and feature extractor for {self.fix_flow_iter} iters.')
                for name, param in self.net_g.named_parameters():
                    if 'spynet' in name or 'sci' in name or 'edvr' in name:
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.pad_scale = self.opt['datasets'].get('val', {}).get('pad_scale',
                 self.opt['datasets'].get('test', {}).get('pad_scale', 4))
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            val_data['voxels'].unsqueeze_(0)

            self.feed_data(val_data)
            val_data['lq'].squeeze_(0)
            val_data['gt'].squeeze_(0)
            val_data['voxels'].squeeze_(0)

            self.test_by_padding()
    ...
    def test_by_padding(self):
        self.net_g.eval()

        B, N, C, H, W = self.lq.shape

        pad_h = (self.pad_scale - H % self.pad_scale) % self.pad_scale
        pad_w = (self.pad_scale - W % self.pad_scale) % self.pad_scale

        # Apply padding if necessary
        if pad_h > 0 or pad_w > 0:
            # Pad (left, right, top, bottom)
            self.lq = F.pad(self.lq, (0, pad_w, 0, pad_h), mode='constant', value=0)
            self.voxels = F.pad(self.voxels, (0, pad_w, 0, pad_h), mode='constant', value=0)

        with torch.no_grad():
            # Pass the padded input through the network
            output = self.net_g(self.lq, self.voxels)
            output = output[0] if isinstance(output, tuple) else output

            # Calculate the amount to crop from the output to match the original size
            if pad_h > 0 or pad_w > 0:
                output = output[:, :, :, :H * self.scale, :W * self.scale]  # Crop back to the original size

            # Assign the processed output
            self.output = output
